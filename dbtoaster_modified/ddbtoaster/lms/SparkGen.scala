...
class LMSSparkGen(cgOpts: CodeGenOptions) extends DistributedM3Gen(cgOpts, SparkExpGen) {  
  ...
  protected def emitMainClass(s0: System): String = {
    ...
    s"""|import ddbt.lib._ 
        ...
        |  var enumPoints: String = ""
        |  var enumPointsList: List[Int] = null
        |  
        ...
        |
        |  def execute(args: Array[String], listener: ExperimentHelperListener) =
        ...
        |        run[$sSparkObject]($sSources, parallelMode, timeout, batchSize, listener)
        |      }, listener)
        | 
        ...
        |    enumPoints = argMap.get("enumPoints").map(_.asInstanceOf[String]).getOrElse(enumPoints)
        |    noOutput = argMap.contains("noOutput")
        |
        ...
        |
        |    enumPointsList = enumPoints.split(",").filter(_ != "").map(_.toInt).toList
        |
        |    disableLog4j()
        |
  ...
  protected def emitActorClass(s0: System, updateBlocks: List[StatementBlock], 
    ...
    val sProcessBatches = if (isInputDistributed) "processBatches(numBatches, resolveExperimentHelperActor())" else ""
    ...
    s"""|class $sSparkObject extends Actor {
        ...
        |        sender ! (StreamStat(endTime - startTime, 0, 0), List())
        |      } else {
        |        val snap = getSnapshot
        |        endTime = System.nanoTime
        |        sender ! snap
        |      }
        |      resolveExperimentHelperActor() ! ddbt.lib.Helper.ExecutionTimeEvent(((endTime - startTime) / 1000000L).toDouble)
        |      logWriter.println("Execution Time: " + ((endTime - startTime) / 1000000000d).formatted("%.2f"), true)
        |      destroyContext()
        |    case GetSnapshot(_) => 
        |      sender ! getSnapshot
        |$sEventHandler
        ...
        |$sConstants
        |
        |  // resolve ExperimentHelperActor ref from ctx
        |  def resolveExperimentHelperActor(): akka.actor.ActorRef = {
        |    val timeout = akka.util.Timeout(10, java.util.concurrent.TimeUnit.SECONDS)
        |    scala.concurrent.Await.result(context.actorSelection("../" + ddbt.lib.Helper.ExperimentHelperActorName).resolveOne()(timeout), timeout.duration)
        |  }
        |}
        |""".stripMargin
  }
  ...
  protected def emitActorBody(updateBlocks: List[StatementBlock],
    ...
    val sProcessFn =
      if (isInputDistributed) {
        ...
        s"""|def processBatches(numBatches: Int, actorRef: akka.actor.ActorRef) = {
            |  if (enumPointsList.nonEmpty) {
            |    logWriter.println("### FULL ENUM AT : " + enumPointsList.mkString("(", ",", ")"), true)
            |  }
            |
            |  logWriter.println("### PROCESSING START: " + numBatches, true)
            |  var runningTime = 0L
            |  for (i <- 0 until numBatches) {
            |    if (enumPointsList.nonEmpty && i >= enumPointsList.head) {
            |      logWriter.println("### FULL ENUM AT BATCH #" + i, true)
            |      val snap = getSnapshot
            |      if (!noOutput)
            |        actorRef ! getSnapshot
            |      enumPointsList = enumPointsList.tail
            |    }
            |
