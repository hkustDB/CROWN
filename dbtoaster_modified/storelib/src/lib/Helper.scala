...
object Helper {
  ...
  case class ExecutionTimeEvent(time: Double)
  class ExperimentHelperActor(listener: ExperimentHelperListener) extends Actor {
    override def receive: Receive = {
      case snapshot: (StreamStat, List[_]) => listener.onSnapshot(snapshot._2)
      case ExecutionTimeEvent(t) => listener.onExecutionTime(t)
    }
  }
  val ExperimentHelperActorName = "ExperimentHelper"

  trait ExperimentHelperListener {
    def onSnapshot(snapshot: List[Any])
    def onExecutionTime(time: Double)
  }

  def run[Q <: akka.actor.Actor](streams: Streams, parallel: Int = 0,
                                 timeoutMilli: Long = 0L, batchSize: Int = 0, listener: ExperimentHelperListener)
                                (implicit cq: ClassTag[Q]) = {
    val system = actorSys()
    val query = system.actorOf(Props[Q] /* .withDispatcher("bounded-mailbox") */, "Query")

    system.actorOf(Props(new ExperimentHelperActor(listener)), ExperimentHelperActorName)
  ...
  def bench(args: Array[String], run: (String, Int, Long, Int) => (StreamStat, List[Any]), listener: ExperimentHelperListener = null) {
    ...
    datasets.foreach { dataset =>
      ...
      if (res0 != null && listener != null && !noOutput) {
        listener.onSnapshot(res0)
      }
    }
  }
  ...
  def parseArgs(args: Array[String]): Map[String, Any] = {
    ...
    List(
      ...
      parseShort("-ep", None, x => Some(x)) match {
        case Some(x) => List(("enumPoints", x)) case None => Nil
      },
