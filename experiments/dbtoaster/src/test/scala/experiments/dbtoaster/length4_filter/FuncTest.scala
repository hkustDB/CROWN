package experiments.dbtoaster.length4_filter
...
class FuncTest {
    ...
    @Before
    def prepare(): Unit = {
        properties.load(getClass.getClassLoader.getResourceAsStream("length4_filter.config.properties"))
    }
    ...
    def trivialApproach(properties: Properties): Map[(Int, Int, Int, Int, Int), Int] = {
        ...
        val list = for {
            ...
            via3 <- graph.getOrElse(via2, List.empty[Int])
            dst <- graph.getOrElse(via3, List.empty[Int])
            if dst > filterValue
        } yield (src, via1, via2, via3, dst)

        list.groupBy(t => t).mapValues(s => s.size)
    }

    def dbtoasterApproach(properties: Properties): Map[(Int, Int, Int, Int, Int), Int] = {
        var result: Map[(Int, Int, Int, Int, Int), Int] = null
        val listener = new ExperimentHelperListener {
            override def onSnapshot(snapshot: List[Any]): Unit = {
                result = snapshot.head.asInstanceOf[Map[(Long, Long, Long, Long, Long), Long]]
                    .map(t => (t._1._1.toInt, t._1._2.toInt, t._1._3.toInt, t._1._4.toInt, t._1._5.toInt) -> t._2.toInt)
            }
