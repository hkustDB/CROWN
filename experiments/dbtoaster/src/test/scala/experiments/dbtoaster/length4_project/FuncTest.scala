package experiments.dbtoaster.length4_project
...
class FuncTest {
    ...
    @Before
    def prepare(): Unit = {
        properties.load(getClass.getClassLoader.getResourceAsStream("length4_project.config.properties"))
    }
    ...
    def trivialApproach(properties: Properties): Map[(Int, Int, Int), Int] = {
        ...
        } yield (via1, via2, via3)

        list.distinct.map(t => (t, 1)).toMap
    }

    def dbtoasterApproach(properties: Properties): Map[(Int, Int, Int), Int] = {
        var result: Map[(Int, Int, Int), Int] = null
        val listener = new ExperimentHelperListener {
            override def onSnapshot(snapshot: List[Any]): Unit = {
                result = snapshot.head.asInstanceOf[Map[(Long, Long, Long), Long]]
                    .map(t => (t._1._1.toInt, t._1._2.toInt, t._1._3.toInt) -> (if (t._2.toInt > 0) 1 else 0))
            }
