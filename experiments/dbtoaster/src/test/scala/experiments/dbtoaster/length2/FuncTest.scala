package experiments.dbtoaster.length2
...
class FuncTest {
    ...
    @Before
    def prepare(): Unit = {
        properties.load(getClass.getClassLoader.getResourceAsStream("length2.config.properties"))
    }
    ...
    def testFunc(): Unit = {
        ...
        // {(1, 2, 3) -> 2, (2, 3, 4) -> 1}
        val snapshot = list.groupBy(t => t).mapValues(s => s.size)
        ...
        val listener = new ExperimentHelperListener {
            override def onSnapshot(snapshot: List[Any]): Unit = {
                result = snapshot.head.asInstanceOf[Map[(Long, Long, Long), Long]]
                    .map(t => (t._1._1.toInt, t._1._2.toInt, t._1._3.toInt) -> t._2.toInt)
            }
        ...
        assertTrue(snapshot.size == result.size)
        snapshot.foreach { case (tuple, cnt) => assertTrue(result(tuple) == cnt) }
    }
}
