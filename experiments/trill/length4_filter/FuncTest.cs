...
namespace Length4_filter.Test {
    public class FuncTest {
        //[Fact]
        public void TestFunc() {
            string path = ConfigurationManager.AppSettings["length4_filter.func.path"];
            ...
            IEnumerable<Path> result = edges.Keys.SelectMany(
                src => edges[src].SelectMany(
                    via1 => edges.GetValueOrDefault(via1, empty).SelectMany(
                        via2 => edges.GetValueOrDefault(via2, empty).SelectMany(
                            via3 => edges.GetValueOrDefault(via3, empty).Where(dst => dst > 100080).Select(
                                dst => new Path(src, via1, via2, via3, dst))))));
