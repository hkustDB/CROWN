...
namespace Length4_project.Test {
    public class FuncTest {
        //[Fact]
        public void TestFunc() {
            string path = ConfigurationManager.AppSettings["length4_project.func.path"];
                ...
                src => edges[src].SelectMany(
                    via1 => edges.GetValueOrDefault(via1, empty).SelectMany(
                        via2 => edges.GetValueOrDefault(via2, empty).SelectMany(
                            via3 => edges.GetValueOrDefault(via3, empty).Select(
                                dst => new Path(via1, via2, via3))))));
