...
namespace Length3_filter2.Test {
    public class FuncTest {
        [Fact]
        public void TestFunc() {
            string path = ConfigurationManager.AppSettings["length3_filter.func.path"];
            ...
            IEnumerable<Path> result = edges.Keys.SelectMany(
                src => edges[src].SelectMany(
                    via1 => edges.GetValueOrDefault(via1, empty).SelectMany(
                        via2 => edges.GetValueOrDefault(via2, empty).Where(dst => dst > 100080).Select(
                            dst => new Path(src, via1, via2, dst)))));
        ...
        private Dictionary<Path, int> TrillApproach(string path) {
            ...
            List<string[]> result = Query.Execute(path, 1, 100080, 1);
            result.ForEach(strs => {
                Assert.True(strs[0].Equals("Start") || strs[0].Equals("End"));
                bool isStart = strs[0].Equals("Start");
                Path p = new Path(int.Parse(strs[3]), int.Parse(strs[4]), int.Parse(strs[5]), int.Parse(strs[6]));
                if (isStart) {
