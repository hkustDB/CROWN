package experiments.data.validate.dbtoastercpp.length4
...
object ResultValidator extends Validator {
    ...
    def trivialApproach(path: String): Map[(Int, Int, Int, Int, Int), Int] = {
    ...
    def dbtoastercppApproach(path: String): Map[(Int, Int, Int, Int, Int), Int] = {
        ...
        (for {
            ...
            val via3 = (item \ "C_DST").text.toInt
            val dst = (item \ "D_DST").text.toInt
            val cnt = (item \ "__av").text.toInt
            (src, via1, via2, via3, dst) -> cnt
        }).toMap
    }
}
