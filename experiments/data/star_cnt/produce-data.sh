...
log_file="${PARENT_PATH}/log/produce-data-star_cnt.log"
...
java -jar ${tools_home}/target/data-tools.jar -p "star_cnt" "${SCRIPT_PATH}/${experiment_name}" "func" >> ${log_file} 2>&1
...
java -jar "${tools_home}/target/data-tools.jar" -ft "${func_raw_path}" "${func_output_path}" "${window_factor}" >> ${log_file} 2>&1
