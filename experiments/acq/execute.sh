...
delta_enable=$(prop 'acq.delta.enum.enable' 'false')
full_enable=$(prop 'acq.full.enum.enable' 'true')

filter_value=$(prop 'filter.condition.value' '-1')

cd "${acq_home}"
if [[ ${acq_mode} = 'minicluster' ]]; then
    acq_class_name=$(prop "minicluster.entry.class")
    ...
    if [[ ${filter_value} -ge 0 ]]; then
        timeout -s SIGKILL 14400s taskset -c "${available_cores}" java -Xms128g -Xmx128g -DexecutionTimeLogPath=${SCRIPT_PATH}/log/execution-time.log -cp "target/ACQ-1.0-SNAPSHOT.jar" ${acq_class_name} "--path" "${input_path}" "--graph" "${input_file_name}" "--parallelism" "${parallelism}" "--deltaEnumEnable" "${delta_enable}" "--fullEnumEnable" "${full_enable}" "--n" "${filter_value}" >> ${execute_log} 2>&1
    else
        timeout -s SIGKILL 14400s taskset -c "${available_cores}" java -Xms128g -Xmx128g -DexecutionTimeLogPath=${SCRIPT_PATH}/log/execution-time.log -cp "target/ACQ-1.0-SNAPSHOT.jar" ${acq_class_name} "--path" "${input_path}" "--graph" "${input_file_name}" "--parallelism" "${parallelism}" "--deltaEnumEnable" "${delta_enable}" "--fullEnumEnable" "${full_enable}" >> ${execute_log} 2>&1
    fi
else
    acq_test_name=$(prop "test.entry.class")
    timeout -s SIGKILL 14400s mvn "test" "-Dsuites=${acq_test_name}" "-Dconfig=srcFile=${input_file},deltaEnumEnable=${delta_enable},fullEnumEnable=${full_enable}" >> ${execute_log} 2>&1
fi