package experiments.flink.snb3_window;
...
public class Query implements Executable {
    ...
    public List<Tuple7<Boolean, Integer, Integer, Integer, Integer, Integer, String>> exec() throws Exception {
        ...
        String createTableMessage = resourcesUtil.readResourceFile("snb3_window/message.sql");
        tableEnv.executeSql(createTableMessage);

        String createViewMessageWindowed = resourcesUtil.readResourceFile("snb3_window/messageWindowed.sql");
        tableEnv.executeSql(createViewMessageWindowed);

        String createTablePerson = resourcesUtil.readResourceFile("snb3_window/person.sql");
        tableEnv.executeSql(createTablePerson);

        String createViewPersonWindowed = resourcesUtil.readResourceFile("snb3_window/personWindowed.sql");
        tableEnv.executeSql(createViewPersonWindowed);

        String createTableKnows = resourcesUtil.readResourceFile("snb3_window/knows.sql");
        tableEnv.executeSql(createTableKnows);

        String createViewKnowsWindowed = resourcesUtil.readResourceFile("snb3_window/knowsWindowed.sql");
        tableEnv.executeSql(createViewKnowsWindowed);

        String query = resourcesUtil.readResourceFile("snb3_window/query.sql");
