...
public class Query implements Executable {
    ...
    public List<Tuple5<Boolean, Integer, Integer, Integer, String>> exec() throws Exception {
        ...
        String createTableGraph = resourcesUtil.readResourceFile("length4_project/graph.sql");
        tableEnv.executeSql(createTableGraph);

        String createViewGraphWindowed = resourcesUtil.readResourceFile("length4_project/window.sql");
        tableEnv.executeSql(createViewGraphWindowed);

        String query = resourcesUtil.readResourceFile("length4_project/query.sql");
