...
public class Query implements Executable {
    ...
    public List<Tuple4<Boolean, Integer, Long, String>> exec() throws Exception {
        ...
        String createTableGraph = resourcesUtil.readResourceFile("star_cnt/graph.sql");
        tableEnv.executeSql(createTableGraph);

        String createViewGraphWindowed = resourcesUtil.readResourceFile("star_cnt/window.sql");
        tableEnv.executeSql(createViewGraphWindowed);

        String query = resourcesUtil.readResourceFile("star_cnt/query.sql");
        ...
        if (properties.getProperty("discard.tuples.in.sink", "true").equals("true")) {
            ...
            List<Tuple4<Boolean, Integer, Long, String>> list = new LinkedList<>();
            result.executeAndCollect().forEachRemaining(row ->
                    list.add(new Tuple4<>(
                            row.f0,
                            (Integer) row.f1.getField(0),
                            (Long) row.f1.getField(1),
