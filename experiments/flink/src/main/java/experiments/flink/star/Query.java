...
public class Query implements Executable {
    ...
    public List<Tuple7<Boolean, Integer, Integer, Integer, Integer, Integer, String>> exec() throws Exception {
        ...
        String createTableGraph = resourcesUtil.readResourceFile("star/graph.sql");
        tableEnv.executeSql(createTableGraph);

        String createViewGraphWindowed = resourcesUtil.readResourceFile("star/window.sql");
        tableEnv.executeSql(createViewGraphWindowed);

        String query = resourcesUtil.readResourceFile("star/query.sql");
