package experiments.flink.dumbbell;
...
public class Query implements Executable {
    ...
    public List<Tuple4<Boolean, Integer, Integer, String>> exec() throws Exception {
        ...
        String createTableGraph = resourcesUtil.readResourceFile("dumbbell/graph.sql");
        tableEnv.executeSql(createTableGraph);

        String createGraphWindowed = resourcesUtil.readResourceFile("dumbbell/window.sql");
        tableEnv.executeSql(createGraphWindowed);

        String createViewTriangle = resourcesUtil.readResourceFile("dumbbell/triangle.sql");
        tableEnv.executeSql(createViewTriangle);



        String query = resourcesUtil.readResourceFile("dumbbell/query.sql");
