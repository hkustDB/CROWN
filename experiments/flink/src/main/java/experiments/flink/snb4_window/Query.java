package experiments.flink.snb4_window;
...
public class Query implements Executable {
    ...
    public List<Tuple7<Boolean, Integer, Integer, Integer, Integer, Integer, String>> exec() throws Exception {
        ...
        String createTableMessage = resourcesUtil.readResourceFile("snb4_window/message.sql");
        tableEnv.executeSql(createTableMessage);

        String createViewMessageWindowed = resourcesUtil.readResourceFile("snb4_window/messageWindowed.sql");
        tableEnv.executeSql(createViewMessageWindowed);

        String createTableMessageTag = resourcesUtil.readResourceFile("snb4_window/messagetag.sql");
        tableEnv.executeSql(createTableMessageTag);

        String createViewMessageTagWindowed = resourcesUtil.readResourceFile("snb4_window/messagetagWindowed.sql");
        tableEnv.executeSql(createViewMessageTagWindowed);

        String createTableKnows = resourcesUtil.readResourceFile("snb4_window/knows.sql");
        tableEnv.executeSql(createTableKnows);

        String createViewKnowsWindowed = resourcesUtil.readResourceFile("snb4_window/knowsWindowed.sql");
        tableEnv.executeSql(createViewKnowsWindowed);

        String createTableTag = resourcesUtil.readResourceFile("snb4_window/tag.sql");
        tableEnv.executeSql(createTableTag);

        String query = resourcesUtil.readResourceFile("snb4_window/query.sql");
