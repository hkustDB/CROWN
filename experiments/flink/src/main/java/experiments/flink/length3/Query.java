...
public class Query implements Executable {
    ...
    public List<Tuple6<Boolean, Integer, Integer, Integer, Integer, String>> exec() throws Exception {
        ...
        String createTableGraph = resourcesUtil.readResourceFile("length3/graph.sql");
        tableEnv.executeSql(createTableGraph);

        String createViewGraphWindowed = resourcesUtil.readResourceFile("length3/window.sql");
        tableEnv.executeSql(createViewGraphWindowed);

        String query = resourcesUtil.readResourceFile("length3/query.sql");
