package experiments.flink.TwoComb;
...
public class Query implements Executable {
    ...
    public List<Tuple6<Boolean, Integer, Integer, Integer, Integer, String>> exec() throws Exception {
        ...
        String createTableGraph = resourcesUtil.readResourceFile("TwoComb/graph.sql");
        tableEnv.executeSql(createTableGraph);

        String createViewGraphWindowed = resourcesUtil.readResourceFile("TwoComb/graphWindowed.sql");
        tableEnv.executeSql(createViewGraphWindowed);

        String createTableV1 = resourcesUtil.readResourceFile("TwoComb/v1.sql");
        tableEnv.executeSql(createTableV1);

        String createViewV1Windowed = resourcesUtil.readResourceFile("TwoComb/v1Windowed.sql");
        tableEnv.executeSql(createViewV1Windowed);

        String createTableV2 = resourcesUtil.readResourceFile("TwoComb/v2.sql");
        tableEnv.executeSql(createTableV2);

        String createViewV2Windowed = resourcesUtil.readResourceFile("TwoComb/v2Windowed.sql");
        tableEnv.executeSql(createViewV2Windowed);

        String query = resourcesUtil.readResourceFile("TwoComb/query.sql");
