...
public class FuncTest {
    ...
    public void testFunc() throws Exception {
        ...
        p.load(FuncTest.class.getClassLoader().getResourceAsStream("length2/func.cfg"));
    ...
    /**
     * flink approach of Length2 experiment
     * @return a Map, key = group(window_start), value = length2 paths in that window
     */
    ...
    public Map<Integer, Set<Tuple3<Integer, Integer, Integer>>> trivialApproach(Properties p) throws IOException {
        ...
        trivialGroups.forEach(group -> {
            ...
            edges.keySet().forEach(src -> {
                List<Integer> vias = edges.get(src);
                vias.forEach(via -> {
                    List<Integer> dsts = edges.getOrDefault(via, new LinkedList<>());
                    dsts.forEach(dst -> {
                        result.add(new Tuple3<>(src, via, dst));
                    });
                });
            });
