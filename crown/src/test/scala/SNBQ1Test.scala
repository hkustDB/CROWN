...
class SNBQ1Test extends AnyFlatSpec with BeforeAndAfterAllConfigMap {
  var srcFile = ""
  ...
  "SNBQ2Test" should "forward values" in {
    ...
    val processFunction = new SNBQ1ProcessFunctions(deltaEnumMode)
    ...
    for (row <- rows) {
      ...
      if (row._1 == "+" || row._1 == "-") {
        ...
        indicator match {
          case "ME" =>
            relation = "message"
            if (cells.length < 14 || cells(13).isEmpty)
              harness.processElement(Payload(action, relation, 0,
                Attributes(Array[Any](cells(2).toLong, cells(9).toLong), Array[String]("MessageID", "IDB")), cnt), cnt)
          case "KN" =>
            relation = "knows"
            harness.processElement(Payload(action, relation, 0,
          ...
          case "PE" =>
            relation = "person"
            harness.processElement(Payload(action, relation, 0,
              Attributes(Array[Any](cells(2).toLong, cells(3), cells(4)), Array[String]("IDB", "firstName", "secondName")), cnt), cnt)
          case _ =>
            cnt = cnt - 1
        }
      } else if (row._1 == "*") {
