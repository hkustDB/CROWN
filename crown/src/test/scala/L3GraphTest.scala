...
class L3GraphTest extends AnyFlatSpec with BeforeAndAfterAllConfigMap {
  var srcFile = ""
  ...
  "PassThroughProcessFunction" should "forward values" in {
    ...
    val processFunction = new L3ProcessFunctions(deltaEnumMode)
  ...
  def feedElements(harness: KeyedOneInputStreamOperatorTestHarness[Any, Payload, String], action: String,
    ...
    harness.processElement(Payload(action, "G1", 0,
      Attributes(fields, Array[String]("A", "B")), 0), ts)
    harness.processElement(Payload(action, "G2", 0,
      Attributes(fields, Array[String]("B", "C")), 0), ts)
    harness.processElement(Payload(action, "G3", 0,
      Attributes(fields, Array[String]("C", "D")), 0), ts)
  }
