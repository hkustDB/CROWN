...
object L3Job {
  ...
  def main(args: Array[String]) {
    ...
    val result = graphStream.keyBy(i=>i._3).process(new L3ProcessFunctions(deltaEnumMode))//.writeAsText(outputpath, WriteMode.OVERWRITE)
        ...
        override def processElement(value: String, ctx: ProcessFunction[String, Payload]#Context, out: Collector[Payload]): Unit = {
          ...
          header match {
            case "+" =>
              ...
              relation = "G3"
              if (cells(1).toInt > nFilter) ctx.output(graphTag, Payload(action, relation, 0.asInstanceOf[Any],
