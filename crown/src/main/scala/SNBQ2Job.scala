...
object SNBQ2Job {
  ...
  def main(args: Array[String]) {
    ...
    val result = inputStream.keyBy(i=>i._3).process(new SNBQ2ProcessFunctions(deltaEnumMode))
    result.addSink(new DiscardingSink[String])
    // execute program
    env.execute("SNB Q2 Program")
  }
        ...
        override def processElement(value: String, ctx: ProcessFunction[String, Payload]#Context, out: Collector[Payload]): Unit = {
          ...
          def outcollect() : Unit = {
            ...
            indicator match {
              case "ME" =>
                relation = "message"
                if (cells(12).isEmpty || cells(12) == "\\N")
                out.collect(Payload(action, relation, KeyList((cells(8).toLong % parallel).toInt),
                    Attributes(Array[Any](cells(1).toLong, cells(8).toLong), Array[String]("MessageID", "IDC")), cnt))
              case "TA" =>
                relation = "tag"
                for (i <- 0 until parallel)
                    out.collect(Payload(action, relation, KeyList(i),
                      Attributes(Array[Any](cells(0).toLong), Array[String]("TagID")), cnt))
              case "KN" =>
                relation = "knows2"
                out.collect(Payload(action, relation, KeyList((cells(2).toLong % parallel).toInt),
                  Attributes(Array[Any](cells(1).toLong, cells(2).toLong), Array[String]("IDB", "IDC")), cnt))
                relation = "knows1"
                if (cells(1).toLong < 100000) {
                  for (i <- 0 until parallel)
                    out.collect(Payload(action, relation, KeyList(i),
              ...
              case "MT" =>
                ...
                for (i<- 0 until parallel)
                  out.collect(Payload(action, relation, KeyList(i),
                    Attributes(Array[Any](cells(0).toLong, cells(1).toLong), Array[String]("MessageID", "TagID")), cnt))
              case _ =>
                cnt = cnt - 1
            }
          }
