...
object SNBQ4Job {
  ...
  def main(args: Array[String]) {
    ...
    val result = inputStream.keyBy(i=>i._3).process(new SNBQ4ProcessFunctions(deltaEnumMode))
    result.addSink(new DiscardingSink[String])
    // execute program
    env.execute("SNB Q4 Program")
  }
              ...
              case "TA" =>
                ...
                for (i <- 0 until parallel)
                    out.collect(Payload(action, relation, KeyList(i),
                      Attributes(Array[Any](cells(0).toLong, cells(1)), Array[String]("TagID", "T_Name")), cnt))
              case "KN" =>
                relation = "knows"
