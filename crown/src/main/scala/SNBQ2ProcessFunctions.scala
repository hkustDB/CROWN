...
class SNBQ2ProcessFunctions(val deltaEnumMode: Int) extends BasedProcessFunction[Any, Payload, String](0, 0, "L3ProcessFunction") {
  ...
  var messageTag : middleRelation = null
  var tag : leafRelation = null
  ...
  override def initstate(): Unit = {
    ...
    messageTag =  new middleRelation("messageTag", Array("MessageID"), Message, deltaEnumMode, 1)
    tag = new leafRelation("Tag", Array("TagID"), messageTag, deltaEnumMode)

    tag.initState(getRuntimeContext, attributeDescriptor, setAttributeDescriptor)
    ...
    Message.addConnection(Array("MessageID"), messageTag)
    messageTag.addConnection(Array("TagID"), tag)
