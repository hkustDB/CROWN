...
class L3ProjectionProcessFunctions(val deltaEnumMode: Int) extends BasedProcessFunction[Any, Payload, String](0, 0, "L3ProcessFunction") {
  ...
  override def initstate(): Unit = {
    ...
    middle =  new middleRelation("G2", Array("B", "C"), null, deltaEnumMode)
    left = new leafRelation("G1", Array("B"), middle, deltaEnumMode)
    right =  new leafRelation("G3", Array("C"), middle, deltaEnumMode)
    ...
    middle.addConnection(Array("C"), right)
  }
  ...
  override def process(value_raw: Payload, ctx: KeyedProcessFunction[Any, Payload, String]#Context, out: Collector[String]): Unit = {
    ...
    if (value_raw._1 == "Insert") {
      ...
      if (value_raw._2 == "G2") middle.insert(value_raw._4)
      if (value_raw._2 == "G3") right.insert(value_raw._4)
    } else {
      if (value_raw._1 == "Delete") {
        if (value_raw._2 == "G1") left.delete(value_raw._4)
        if (value_raw._2 == "G2") middle.delete(value_raw._4)
        if (value_raw._2 == "G3") right.delete(value_raw._4)
      } else {
  ...
  class leafRelation(name : String, p_keys : Array[String], next : Relation, override val deltaEnumMode: Int, isOutput : Boolean = false) extends Relation(name, p_keys, next, deltaEnumMode, 0, isOutput) {
  ...
  class middleRelation(name : String, p_keys : Array[String], next : Relation, override val deltaEnumMode: Int, isOutput : Boolean = true) extends Relation(name, p_keys, next, deltaEnumMode, 2, isOutput) {
