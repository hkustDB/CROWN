...
object SNBQ3Job {
  ...
  def main(args: Array[String]) {
    ...
    val result = inputStream.keyBy(i=>i._3).process(new SNBQ3ProcessFunctions(deltaEnumMode))
    result.addSink(new DiscardingSink[String])
    // execute program
    env.execute("SNB Q3 Program")
  }
            ...
            indicator match {
              ...
              case "PE" =>
                ...
                out.collect(Payload(action, relation, KeyList((cells(1).toLong % parallel).toInt),
                  Attributes(Array[Any](cells(1).toLong, cells(2), cells(3)), Array[String]("IDC", "firstName", "secondName")), cnt))
              case _ =>
                cnt = cnt - 1
            }
          }
