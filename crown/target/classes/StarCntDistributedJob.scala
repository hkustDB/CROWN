...
object StarCntDistributedJob {
  ...
  def main(args: Array[String]) {
    ...
    val result = graphStream.keyBy(i=>i._3).process(new StarCntProcessFunctions(deltaEnumMode))
    result.addSink(new DiscardingSink[String])
    // execute program
    env.execute("Star Count Program")
  }
    ...
    val T : DataStream[Payload] = data
      .process( new ProcessFunction[String, Payload] {
        override def processElement(value: String, ctx: ProcessFunction[String, Payload]#Context, out: Collector[Payload]): Unit = {
          val header = value.substring(0, 1)
          var relation = ""
          var action = ""
          def outcollect() : Unit = {
            val cells: Array[String] = value.substring(2).split(",")
            relation = "R1"
            cnt = cnt + 1
