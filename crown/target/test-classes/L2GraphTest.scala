...
class L2GraphTest extends AnyFlatSpec with BeforeAndAfterAllConfigMap {
  var srcFile = ""
  ...
  "PassThroughProcessFunction" should "forward values" in {
    ...
    val processFunction = new L2ProcessFunctions(deltaEnumMode)
  ...
  def feedElements(harness: KeyedOneInputStreamOperatorTestHarness[Any, Payload, String], action: String,
    ...
    harness.processElement(Payload(action, "Left", 0,
      Attributes(fields, Array[String]("A", "B")), 0), ts)
    harness.processElement(Payload(action, "Right", 0,
