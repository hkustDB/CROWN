...
class L4ProjGraphTest extends AnyFlatSpec with BeforeAndAfterAllConfigMap {
  var srcFile = ""
  ...
  "PassThroughProcessFunction" should "forward values" in {
    ...
    val processFunction = new L4ProjectionProcessFunctions(deltaEnumMode)
  ...
  def feedElements(harness: KeyedOneInputStreamOperatorTestHarness[Any, Payload, String], action: String,
    ...
    if (fields(1).asInstanceOf[Int] > 999999999)
      harness.processElement(Payload(action, "R4", 0,
        Attributes(fields, Array[String]("D", "E")), 0), ts)
  }
