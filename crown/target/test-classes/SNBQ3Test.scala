...
class SNBQ3Test extends AnyFlatSpec with BeforeAndAfterAllConfigMap {
  var srcFile = ""
  ...
  "SNBQ3Test" should "forward values" in {

    //instantiate user-defined function
    val deltaEnumMode = if (deltaEnumEnable) 0 /* enum and drop */ else 4 /* do nothing */
    val processFunction = new SNBQ3ProcessFunctions(deltaEnumMode)
      ...
      if (row._1 == "+" || row._1 == "-") {
        ...
        indicator match {
          ...
          case "PE" =>
            relation = "person"
            harness.processElement(Payload(action, relation, 0,
              Attributes(Array[Any](cells(2).toLong, cells(3), cells(4)), Array[String]("IDC", "firstName", "secondName")), cnt), cnt)
          case _ =>
            cnt = cnt - 1
        }
      } else if (row._1 == "*") {
